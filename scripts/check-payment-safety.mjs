...
const checks = [
  ...
  { label: "missed webhook recovery leaves payment pending verification", pattern: /pending_verification/ },
  { label: "duplicate payment returns existing transaction reference", pattern: /txRef: existingPayment\.chapa_ref/ },
  { label: "manual payment check verifies directly with Chapa", pattern: /source:\s*"client_confirm"/ },
...
for (const required of [
  ...
  "missed webhook recovery leaves payment pending verification",
  "duplicate payment returns existing transaction reference",
  "manual payment check verifies directly with Chapa",
  "receipt route has auth",
  "receipt route checks ownership",
  "payment UI uses secured PDF receipt route",
