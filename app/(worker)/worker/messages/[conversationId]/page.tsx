...
export default function WorkerChatPage() {
  ...
  useEffect(() => {
    fetch("/api/conversations")
      .then((r) => r.json())
      .then((d) => {
        const conv = (d.conversations || []).find((c: ConvInfo) => c.id === convId);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (!uploadRes.ok) {
          setError((await uploadRes.json()).error || "Image upload failed");
          setSending(false);
          setUploading(false);
          return;
        }
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-[#0c0c0e] text-white" style={{ zIndex: 100 }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-[#18181b]/95 backdrop-blur-md border-b border-white/5 shrink-0">
        <button onClick={() => router.push("/worker/messages")} className="p-2 hover:bg-white/10 rounded-full transition-colors shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold truncate">{convInfo?.other_name || "Client"}</span>
          ...
          <p className="text-[11px] text-zinc-400 truncate">Client • <span className="text-green-400">Online</span></p>
        </div>
      </div>
              ...
              <div className={`flex flex-col gap-1 max-w-[75%] ${isMe ? "items-end" : "items-start"}`}>
                {msg.image_url && (
                  <a href={msg.image_url} target="_blank" rel="noopener noreferrer">
                    <img src={msg.image_url} alt="attachment" className={`max-w-[220px] rounded-2xl object-cover shadow-lg ${isMe ? "rounded-br-sm" : "rounded-bl-sm"}`} />
                  </a>
                )}
                {msg.body && (
                  <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${isMe ? "bg-[#2dd4bf] text-black rounded-br-sm" : "bg-[#27272a] text-zinc-100 rounded-bl-sm"}`}>
                    {msg.body}
                  </div>
                )}
      ...
      <div className="px-3 py-3 bg-[#18181b]/95 backdrop-blur-md border-t border-white/5 shrink-0">
        <div className="flex items-center gap-2 bg-[#27272a] rounded-2xl px-3 py-2 border border-white/5">
          <button onClick={() => fileRef.current?.click()} className="p-2 hover:bg-white/10 rounded-xl transition-colors shrink-0">
