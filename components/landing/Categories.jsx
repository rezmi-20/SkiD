...
export default function Categories({ t }) {
  return (
    <section id="services" className="py-24 px-6 md:px-12 lg:px-20 bg-[#09090b]">
      <div className="max-w-[95%] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-16 text-left">
          <div className="max-w-2xl">
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-white uppercase mb-4 leading-[0.9]">
              {t("categories.explore").split(" ")[0]} <span className="text-green-400 italic">{t("categories.explore").split(" ").slice(1).join(" ")}</span>
            </h2>
            <p className="text-zinc-500 text-base md:text-lg font-medium">
              {t("categories.subtitle")}
            </p>
          </div>
          <a href="/client/search" className="bg-white/5 border border-white/10 text-white px-8 h-14 flex items-center justify-center rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all backdrop-blur-xl active:scale-95">
          ...
          {categories.map((cat, i) => {
            ...
            return (
              <motion.a
                ...
                whileHover={{ y: -4, scale: 1.01, borderColor: "rgba(74, 222, 128, 0.3)" }}
                transition={{ delay: i * 0.05, duration: 0.5 }}
                viewport={{ once: true }}
                className={`relative group rounded-[2.5rem] overflow-hidden border border-white/5 bg-[#0c0c0e] shadow-2xl transition-all duration-300 ${spans}`}
              >
                {/* Grayscale filter removed from category background image */}
                <img 
                  src={cat.image} 
                  className="absolute inset-0 w-full h-full object-cover opacity-35 group-hover:scale-105 transition-all duration-1000 ease-out" 
                  alt={cat.id} 
                />
                ...
                <div className="absolute top-6 left-6 flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/40 backdrop-blur-md border border-white/10 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                   ...
                   <span className="text-[8px] font-black text-white uppercase tracking-widest">{t("categories.active_pros")}</span>
                </div>

                <div className="absolute bottom-6 left-6 right-6 text-left">
                   <div className={`flex items-end justify-between ${i === 0 ? 'mb-4' : 'mb-2'}`}>
                      <div className="space-y-1">
                         <h3 className={`${i === 0 ? 'text-2xl md:text-3xl' : 'text-lg md:text-xl'} font-black text-white uppercase tracking-tighter leading-none`}>
                            {t(`categories.${cat.id}`)}
                         </h3>
                         <p className="text-[9px] font-black text-green-400 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all duration-500">
                            {t("categories.discover_experts")}
                         </p>
                      </div>
                      <div className="w-10 h-10 bg-white/10 backdrop-blur-xl border border-white/10 rounded-xl flex items-center justify-center text-white group-hover:bg-green-400 group-hover:text-black transition-all duration-500 flex-shrink-0">
                         {cat.icon}
                      </div>
                   </div>
                </div>
